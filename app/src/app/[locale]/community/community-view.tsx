...
export default function CommunityView({ courseCards }: { courseCards: CourseCardData[] }) {
  ...
  useEffect(() => {
    ...
    if (isNew === "true") {
      ...
      setNewType("question");
      if (courseSlug) {
        const course = courseCards.find((c) => c.slug === courseSlug);
        if (course) {
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {post.courseId && (
                          <Badge variant="outline" className="text-[10px]">
                            {courseCards.find((c) => c.slug === post.courseId)?.title ?? post.courseId}
                          </Badge>
                        )}
                      </div>
